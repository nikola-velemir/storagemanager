...
{
    public class InvoiceServiceTest : IAsyncLifetime
    {
        ...
        public async Task CreateTest_InvalidDocumentId()
        {
            ...
            _mechanicalComponentRepository.Verify(repo => repo.CreateFromExtractionMetadata(It.IsAny<List<ExtractionMetadata>>()), Times.Never);
            _invoiceItemRepository.Verify(repo => repo.Create(It.IsAny<InvoiceItemModel>()), Times.Never);
        }
        [Fact(DisplayName = "Create test - Invalid document id")]
        ...
        {
            _invoiceRepository.Setup(repo => repo.FindByDocumentId(VALID_DOCUMENT.Id)).ReturnsAsync(VALID_INVOICE);
            _invoiceRepository.Setup(repo => repo.FindByDocumentId(INVALID_GUID)).ReturnsAsync((InvoiceModel?)null);
            _mechanicalComponentRepository.Setup(repo => repo.CreateFromExtractionMetadata(METADATA_LIST)).ReturnsAsync(COMPONENTS);
            _mechanicalComponentRepository.Setup(repo => repo.FindByIdentifier("MC-111")).ReturnsAsync(COMPONENTS.Find(c => c.Identifier.Equals("MC-111")));
            _mechanicalComponentRepository.Setup(repo => repo.FindByIdentifier("MC-4001")).ReturnsAsync(COMPONENTS.Find(c => c.Identifier.Equals("MC-4001")));
        }
        public Task InitializeAsync()
        {
            _invoiceItemRepository = new Mock<IInvoiceItemRepository>();
            _mechanicalComponentRepository = new Mock<IMechanicalComponentRepository>();
            _invoiceRepository = new Mock<IInvoiceRepository>();
            MockRepository();
            _service = new InvoiceService(_invoiceItemRepository.Object, _mechanicalComponentRepository.Object, _invoiceRepository.Object);
            return Task.CompletedTask;
        }
    }
}
