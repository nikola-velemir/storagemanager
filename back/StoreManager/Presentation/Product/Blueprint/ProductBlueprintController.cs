...
[Authorize]
[Route("api/products")]
public class ProductBlueprintController(IMediator mediator) : ControllerBase
{
    [HttpPost]
    public async Task<IActionResult> CreateProduct([FromBody] ProductCreateRequestDto request)
    {
        await mediator.Send(new CreateProductBlueprintCommand(request.Name, request.Identifier, request.Description,
            request.Components));
        return Ok();
    }
    ...
    {
        var result = await mediator.Send(new FindFilteredProductBlueprintsQuery(productInfo, dateCreated, pageNumber, pageSize));
        return Ok(result);
    }
